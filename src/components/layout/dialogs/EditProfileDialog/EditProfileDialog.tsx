...
const EditProfileDialog = () => {
  ...
  const [authUser, setAuthUser] = useAuthStore((s) => [s.authUser, s.setAuthUser])

  const { initialValue, dialogIsOpen, closeDialog } = useEditProfileDialogStore()
  ...
  const { watch, control, setValue, handleSubmit, reset } = useForm<AuthUserGetDto>({
  ...
  const handleFileUpload = (file: File) => {
    ...
    myAxios
      ...
      .then((res) => {
        ...
        if (groupId) queryClient.invalidateQueries(queryKeys.groupMembers(groupId))
      })
      .catch((err) => {
        setErrorMessage(
